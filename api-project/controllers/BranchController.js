...
const createBranch = async(req,res)=>{
    try{
     ...
     res.status(200).send({status:true, message:'Branch created successfully' , data: '' })
    }catch(err){
        console.log(err)
    ...
    try{
        ...
        Q.all([
            ...
            .then(function(branches){
                ...
                return  res.status(200).send({status:true, message:'Branch created successfully' , data: branches[1], totalCount: branches[0] });
            })
            .catch(function(err){
